...
#
df <- df %>%
    mutate( relative_age_percentile_count =
        100 * age_percentile_group_count / decade_count )
...
ggplot(
    ...
    aes(
        x = percentile,
        y = relative_age_percentile_count,
        group = decade,
        color = decade)
    ) +
    geom_line(lwd = 0.3) +
    xlab("Performance percentile") +
    ylab("Proportion of participants in age group (%)") +
    ggtitle("Relative perfomance distribution for age groups")
```